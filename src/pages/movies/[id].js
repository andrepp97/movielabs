...
const MovieDetails = () => {
    ...
    const getMovieDetails = useCallback(async (signal) => {
        const result = await fetch(process.env.NEXT_PUBLIC_URL + `/${id}?api_key=${process.env.NEXT_PUBLIC_API_KEY}&language=en-US&page=1`, { signal })
        const data = await result.json()
        setDetails(data)
    }, [id])

    const getMovieVideo = useCallback(async (signal) => {
        const result = await fetch(process.env.NEXT_PUBLIC_URL + `/${id}/videos?api_key=${process.env.NEXT_PUBLIC_API_KEY}&language=en-US&page=1`, { signal })
        const data = await result.json()
    ...
    const getMovieCast = useCallback(async (signal) => {
        const result = await fetch(process.env.NEXT_PUBLIC_URL + `/${id}/credits?api_key=${process.env.NEXT_PUBLIC_API_KEY}&language=en-US&page=1`, { signal })
        const data = await result.json()
        setCasts(data.cast)
    }, [id])
    ...
    const getSimilarMovies = useCallback(async (signal) => {
        ...
        const data = await result.json()
        const temp = data?.results?.filter((item) => item.poster_path && (item.release_date || item.first_air_date))
        const sortedData = temp.sort((a, b) => b.vote_average - a.vote_average).slice(0, 10)
        setSimilar(sortedData)
    }, [id])

    const getMovieReviews = useCallback(async (signal) => {
        const result = await fetch(process.env.NEXT_PUBLIC_URL + `/${id}/reviews?api_key=${process.env.NEXT_PUBLIC_API_KEY}&language=en-US&page=1`, { signal: signal })
        const data = await result.json()
