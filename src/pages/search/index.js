...
const Search = () => {
    ...
    const searchMovies = useCallback(async () => {
        ...
        const result = await fetch(process.env.NEXT_PUBLIC_BASE_URL + `/search/multi?api_key=${process.env.NEXT_PUBLIC_API_KEY}&language=en-US&page=1&include_adult=false${query}`)
        const result2 = await fetch(process.env.NEXT_PUBLIC_BASE_URL + `/search/multi?api_key=${process.env.NEXT_PUBLIC_API_KEY}&language=en-US&page=2&include_adult=false${query}`)
        const data = await result.json()
        const data2 = await result2.json()

        // Filtering the result
        let temp = data.results.filter(movie => movie.poster_path && (movie.release_date || movie.first_air_date))
        if (data2.results.length) {
            let temp2 = data2.results.filter(movie => movie.poster_path && (movie.release_date || movie.first_air_date))
